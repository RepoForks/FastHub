...
public class CountBadgeProvider extends BadgeProvider {
    private final HashMap<Integer, Integer> countMap = new HashMap<>();

    public CountBadgeProvider(final BottomNavigation navigation) {
        super(navigation);
    }
    ...
    @Override public void remove(@IdRes final int itemId) {
    ...
    @Override protected Drawable newDrawable(@IdRes final int itemId, final int preferredColor) {
        ...
        return new Badge(Color.parseColor("#FF2A456B"), count);
    }
