...
public class CreateGistPresenter extends BasePresenter<CreateGistMvp.View> implements CreateGistMvp.Presenter {
    ...
    @Override public void onSubmit(@NonNull CreateGistModel model) {
        manageSubscription(RxHelper.getObserver(RestClient.createGist(model))
                .doOnSubscribe(() -> sendToView(CreateGistMvp.View::onShowProgress))
                .doOnNext(gistsModel -> sendToView(view -> view.onSuccessSubmission(gistsModel)))
