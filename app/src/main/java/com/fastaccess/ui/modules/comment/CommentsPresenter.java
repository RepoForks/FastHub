...
import static com.fastaccess.helper.BundleConstant.EDIT_COMMENT_EXTRA;
import static com.fastaccess.helper.BundleConstant.FOR_RESULT_EXTRA;
import static com.fastaccess.helper.BundleConstant.NEW_COMMENT_EXTRA;

/**
 * Created by Kosh on 27 Nov 2016, 1:31 AM
 */

public class CommentsPresenter extends BasePresenter<CommentsMvp.View> implements CommentsMvp.Presenter {
    ...
    @Override public void onEditComment(long id, @Nullable CharSequence savedText, @NonNull String gistId) {
    ...
    @Override public void onSubmitComment(@Nullable CharSequence savedText, @NonNull String gistId) {
        if (!InputHelper.isEmpty(savedText)) {
            CommentRequestModel requestModel = new CommentRequestModel();
            requestModel.setBody(savedText.toString());
    ...
    @Override public void onHandleSubmission(@Nullable CharSequence savedText, @Nullable @BundleConstant.ExtraTYpe String extraType,
        ...
        switch (extraType) {
            case EDIT_COMMENT_EXTRA:
                if (itemId == null) {
                    throw new NullPointerException("itemId is null");
                }
                onEditComment(id, savedText, itemId);
                break;
            case NEW_COMMENT_EXTRA:
                if (itemId == null) {
                    throw new NullPointerException("itemId is null");
                }
                onSubmitComment(savedText, itemId);
                break;
            case FOR_RESULT_EXTRA:
                sendToView(CommentsMvp.View::onSendMarkDownResult);
                break;
        }
    }
}
